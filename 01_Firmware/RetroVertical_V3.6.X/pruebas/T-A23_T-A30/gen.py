...
vals = [c for row in fab for c in row] + list(temp)          # 51, 35 distintos
rnd = []
for i in range(200):
    g = i % 3
    if g == 0:   # c2
...
vals += rnd
json.dump({'vals': vals, 'nfab': 51}, open('vals.json', 'w'))
...
json.dump({'curvas': curvas, 'temps': temps}, open('curvas.json', 'w'))
