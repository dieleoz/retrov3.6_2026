# SPEC — App de usuario (campo) V3.6: flujo y pantallas (r4)

**Nada de esto está medido contra un equipo ni un teléfono; no hay código propio de esta app todavía.**
**r4, 21-sep-2026: cierra el NO APTO del `arquitecto-iot` sobre la r3.** Cambios de fondo: (1) la SPEC
se organiza en **dos incrementos** (§0); (2) respuesta tardía y trama partida con plazo máximo y
silencio de fin, con su fuente (RF-USR-16); (3) la regla del cero pasa a "repetir una vez, y si la
repetición también trae 0, se guarda con motivo" (RF-USR-04); (4) `#GN,NONE#` en "Medir y exportar" ya
no ofrece teclear nada (RF-USR-06); (5) columnas fijas de `medidas.csv` para el incremento 1
(RF-USR-06); (6) el parser trata `#ERR,...#` y el silencio de `#GN#`/`#GC#` (§3); (7) el % se calcula
con una sola división entera, sin el error de trunco-y-resto que daba −51 % en vez de −50 %
(RF-USR-10); (8) `inventario.csv` lleva `serie_equipo_instalacion` y el dictamen ANI exige que coincida
con el equipo conectado (RF-USR-13); (9) la ruta del manual de 2009 y el plan para cerrar C-06 se
corrigen (RF-USR-10, §5); (10) las citas entre comillas atribuidas a Diego que no eran literales se
retiran; lo que sigue como cita literal, va con su punto de `DECISIONES-Diego-2026-09-19.md`. Detalle
numérico y de verificación en `TDD-V3.6.md` §8. Fuentes: `PROTOCOLO-V3.6.md`, `SPEC-V3.6.md`,
`calibracion_v36.c`, `ecuacionesCalibracion.c`, `gui.c` (2020 y V3.6), `SPEC-Registro-Indicador-
Interventoria.md` (SPEC-REG), `08_Senales/senales.csv`, `rtv-1.0` (§3),
...
- **Fichas (TDD §8):** T-USR-01, T-USR-01b, T-USR-02, T-USR-03, T-USR-03b, T-USR-04, T-USR-06,
  T-USR-07, T-USR-19, T-USR-20, T-USR-21, T-USR-22, T-USR-23, T-USR-24.
...
- **Fichas (TDD §8):** T-USR-05, T-USR-08 a T-USR-18.
...
equipo" y **no se mide**. Si `#GN#` o `#GC#` **no contesta nada** dentro del plazo (RF-USR-16), la app
no concluye "3.6/3.6.1": muestra "el equipo no respondió, reintente" y permite reintentar la sonda
antes de decidir; sólo un `#ERR,FORMATO#` explícito dispara "actualice el firmware" (§3, parser). Esto
es distinto de `#GN,NONE#` (equipo 3.6.2 sin serie grabada, RF-USR-06): ahí sí se mide, sin pedir nada
al operador. *Fuente:* `PROTOCOLO-V3.6.md:42` (formato de `#V#`; **cita corregida, r2 decía `:24`**);
...
11a; distinto del 4 de RF-REG-08 general), configurable; media, mínimo, trama cruda. Un `0` se muestra
**"0 (saturado o negativo)"**, nunca oculto ni nulo. **Regla del cero, corregida (H-A3, sustituye a
"repetir siempre"):** si la serie trae algún `0`, se repite **una vez**; si la **segunda** serie
también trae algún `0`, se guarda la fila con `media = 0`, `válido = NO` y `motivo =
"saturado_o_negativo"`, y esa fila cuenta como **NO CUMPLE / bajo referencia** en vez de excluirse:
"no decir null a todo sino indicar que no cumple con una medida estándar" (DECISIONES nota 8,
USR-ALCANCE). **Nunca se promedia excluyendo sólo los ceros** (una serie `0, 0, 330` no da media 330
con n = 1: se repite entera, y si la repetición también trae ceros, se guarda con el motivo). Motivo
técnico del cero: `reflectivityValue` es sin signo; una lectura negativa da un entero enorme, `> 4000`,
y se fuerza a `0` (`ecuacionesCalibracion.c:49-54`, `arreglar_dato`, llamada en `:59` de
`conversionDatoEnviar`); `0` casi siempre es lámina muy degradada o mala lectura. *Fuente:*
`SPEC-V3.6.md:501-503`; `ecuacionesCalibracion.c:49-54`; DECISIONES nota 11a (disparos); nota 8
(motivo de no ocultar el 0).
...
nunca aquí. CSV crudo, sin señal ni cumple/no cumple (eso es de "Señal a señal"). Columnas fijas de
`medidas.csv` para este incremento (M-6): `fecha_hora`, `lat`, `lon`, `gps_estado`, `color`,
`codigo_bt` (el byte `1`-`6` enviado, RF-USR-03), `n` (disparos de la serie), `lecturas` (las `n`
tramas crudas, separadas por `|`), `media` (entera; **redondeo estándar, mitad hacia arriba**, del
...
Diego), `minimo`, `valido` (`SI`/`NO`; `NO` sólo en el caso de cero persistente de RF-USR-04),
`motivo` (vacío o `"saturado_o_negativo"`), `serie_equipo`, `serie_origen` (`"leida"` o `"ninguna"`),
`mac`, `firmware_v` (respuesta completa de `#V#`), `fecha_calibracion`, `vencimiento`,
`estado_calibracion` (`CAL`/`DEF`/`vencida`). Una fila por serie de disparos y color. *Fuente:*
DECISIONES nota 10; SPEC-REG `:566` (RF-REG-01); SPEC-REG `:592` (RF-REG-27, sin credenciales);
`PROTOCOLO-V3.6.md:54` (`#GN,NONE#`); `CLAUDE.md` §5 (L-23, serie/MAC/vencimiento en todo registro
exportado).
...
RF-USR-13: hora, trama enviada, trama recibida (o "timeout"), byte a byte. **Este registro es una
salida para auditoría del operador y de interventoría, no la fuente de verdad de la prueba de la lista
blanca**: la ficha que cierra RF-USR-15 (T-USR-19) asevera contra lo que **recibe el simulador**
directamente, no contra este fichero, porque un fallo en el propio volcado a `tramas.log` daría un
falso verde si se aseverara sólo contra él (TDD T-USR-19). *Fuente:* CLAUDE.md §2 (verificación medida,
no de código); M-6.
...
- **Parámetros de plazo y silencio, con su fuente.** Silencio de fin de respuesta: **180 ms**
  ...
  Pausa mínima antes de un nuevo envío: 1500 ms desde el envío anterior y 600 ms desde el último byte,
  150 ms entre dos tramas `#` con el equipo ya identificado (`SPEC-V3.6.md:441-446`, RF-APP-01).
- **Si vence el plazo de un disparo, esa lectura se anula**: no se cuenta como "0" ni se descarta en
  silencio; se marca "sin respuesta" en el registro y **no se envía ninguna otra trama hasta que pase
  un silencio mayor que un ciclo del firmware** (500 ms, el `delayTimeout` de `gui.c:346` que ya cita
  `SPEC-V3.6.md:440` para el mismo motivo), para no disparar una medida con un byte que el firmware
  seguía procesando.
- **Respuesta tardía (condición 4).** `::<n>` no lleva identificador: lo que llega **fuera de plazo**
  (tras el tiempo máximo de espera de la petición) se descarta y **no se asigna al disparo siguiente**:
  una respuesta tardía de la medida N no se cuenta como respuesta de la medida N+1, ni se promedia con
  ella.
...
- Ficha de respuesta tardía: se envía el disparo N+1 tras el plazo de N; la respuesta de N llega
  después de ese envío → se descarta por fuera de plazo, no se suma a N+1 ni a la media.
...
- *Fuente:* `rtv-1.0:03_App_Movil/RetroV36/app/src/main/java/com/dpi/retrov36/Tramas.java:266,286-298`
  ...
  y plazo); V3.6 `gui.c:342-346` (limpieza de búfer).
...
terminador (RF-USR-16). Cualquier `#ERR,<motivo>#` recibido en respuesta a `#GN#`/`#GC#` se reconoce
genéricamente (el `motivo` es texto corto sin comas, `PROTOCOLO-V3.6.md:71-72`: `PIN`, `BLOQUEADO`,
`FORMATO`, `EEPROM`); en esta app sólo se espera `FORMATO` (equipo sin esas órdenes, RF-USR-01), pero
el parser no debe fallar si llega otro motivo: lo registra en `tramas.log` y trata la sonda como "sin
respuesta útil", igual que un timeout. **Si `#GN#` o `#GC#` no contestan nada** dentro del plazo de
RF-USR-16 (ni `#GN,...#` ni `#ERR,...#`), la app **no** concluye "actualice el firmware": eso es sólo
para un `#ERR,FORMATO#` explícito. Un silencio puede ser el enlace, no la versión: se ofrece reintentar
la sonda (RF-USR-01). *Fuente:* `PROTOCOLO-V3.6.md:70-72` (formato de `motivo`); RF-USR-01, RF-USR-16.
...
- **C-USR-04, cerrada como propuesta.** M-3 (RF-USR-01) fija: se exige firmware 3.6.2; con
  `#ERR,FORMATO#` a `#GN#`/`#GC#` (3.6, 3.6.1), "actualice el firmware" y no se mide; un timeout sin
  `#ERR#` no cuenta como eso (§3, parser). Pendiente de confirmación de Diego, como el resto de las
  propuestas de esta r4.
