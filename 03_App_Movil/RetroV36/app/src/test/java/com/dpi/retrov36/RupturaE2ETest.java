...
public class RupturaE2ETest {
    ...
    void preparar(boolean abrirDosVeces) throws Exception {
        ...
        while ((p = q.siguiente(c.pasos())) != null && n++ < 500) {
            ...
            int[] e = ProtocoloDisparos.efectivo(p, true, "PRECISO", REP);
        ...
        for (String t : REP) {
            Campana.Serie s = c.elegida(t); System.out.print(t + "=" + s.id + " " + ProtocoloDisparos.deSerie(s)[0] + "x" + ProtocoloDisparos.deSerie(s)[1] + "; ");
        }
        System.out.println();
    }
