 ...
 * cerraba el socket con una medida en vuelo) contesta SALTAR a la pregunta pendiente y a las que
 * vengan después EN ESA MISMA MEDIDA, para que {@code sesion.medir()} termine de verdad y suelte el
 * monitor de {@code SesionMedicion.java:100} (sin eso, un {@code synchronized} colgado bloquearía
 * también la medida siguiente, aunque fuera sobre un equipo nuevo). El oyente se registra en
 * {@code onResume()} y se retira en {@code onPause()}, igual que {@link EstadoDeteccion}.</p>
 */
public final class EstadoMedida implements PreguntaOperador {
    ...
    private Decision preguntar(TipoPregunta tipo) {
        ...
        synchronized (this) {
            if (abandonado) {
                return Decision.SALTAR; // abandonar() ya se llamó para esta medida: nunca vuelve a bloquear.
            }
